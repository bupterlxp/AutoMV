...
from gemini_verify import call_gemini 
...
def generate(para,label,qwen_result,name,try_number=1):
    ...
    for prompt_p in image_prompts:
        prompt = prompt_p
        save_folder = f"../result/{name}/picture{try_number}/{para['number']}" 
        retry = 3
        best_dict = {
            "image_url": "",
            "score": 0
        ...
        # 无Gemini_veirfy
        # prompt = prompt_p
        # save_folder = f"/map-vepfs/nicolaus625/m2v/tangxiaoxuan/work/{name}/picture{try_number}/{para['number']}"  # 指定文件夹名
        # if not os.path.exists(save_folder):
        #     os.makedirs(save_folder)       
    ...
    save_folder = f"../result/{name}/camera{try_number}"
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
...
# begin
def generate_one_mv(name ,try_number= 1,start_num=1,end_num=100):
    audio_path_vocal = f"../result/{name}/{name}.mp3"
    audio_path = f"../result/{name}/{name}.mp3"
    lyrics = transcribe_audio(audio_path_vocal)
    songformer = read_json_file(f"/map-vepfs/nicolaus625/m2v/tangxiaoxuan/work/songformer/{name}.json")
    prompt = f"""
...
if __name__ == "__main__":
    for i in [51]:  # 3~57
        ...
        while not success:
            try:
                generate_one_mv(str(i),1,26,26)
                success = True 
            except Exception as e:
                print(f"generate_one_mv({i}) 失败，正在重试... 错误信息: {e}")
    # # 读取所有文件名（只取文件，不取子目录）
    # file_list = [
    ...
    # # 设置线程池大小（可根据CPU核数调整）
    # max_workers = min(4, len(file_list)) or 1

    # results = []
    # with ThreadPoolExecutor(max_workers=max_workers) as executor:
    #     future_to_file = {executor.submit(generate_one_mv, f): f for f in file_list}
